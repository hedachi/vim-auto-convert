" auto_convertのE2Eテスト: vim -N -u NONE -es -S test/test_auto_convert.vim で実行
" 結果は test/results/test_auto_convert_result.txt に出力される
set nocompatible
let s:root = fnamemodify(expand('<sfile>'), ':p:h:h')
execute 'set runtimepath^=' . fnameescape(s:root)
runtime plugin/auto_convert.vim
call mkdir(s:root . '/test/results', 'p')

" テストバッファ準備
enew
call setline(1, ['vimプラグインのメモ', '', 'ここに追記していく'])
" 1回目のTickでベースラインsnapshot採取
AutoConvertNow

" 誤字入りの追記をシミュレート
call setline(2, 'キホ的に適当にtpyoするから')
call append(2, 'kouyatte henkan sinaide kaitemo iiyounisite')

" 2回目のTickでAIに送信
AutoConvertNow

" ジョブ完了待ち（最大30秒）
let s:waited = 0
while s:waited < 300
  if !empty(g:auto_convert_last)
    break
  endif
  sleep 100m
  let s:waited += 1
endwhile

let s:result = ['last=' . string(g:auto_convert_last)]
call extend(s:result, getline(1, '$'))
call writefile(s:result, s:root . '/test/results/test_auto_convert_result.txt')
qall!
